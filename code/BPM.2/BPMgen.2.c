...
// ----------------------IMMISSIONE OPERANDI ED INIZIALIZZAZIONE INGRESSI----------------------
	...
	fpr = fopen ("BPMdata.txt", "r");
	fscanf (fpr, "%d %d %d", &x, &y, &p);
	

	for (i = 2*N-1; i >= 0; i--) {		//conversione prodotto da decimale a binario
		P_binary[i] = p%2;
		p = p/2;
	}
...
//----------------------------------------------------------------------------------
	...
	for (i = n-1, j = 0; i >= 0, j < N; i--, j++) {			//inizializzazione valori di X sulla netlist
		fprintf(fp, "VinX%d X%d 0 %d\n", i, i, X_binary[j]);
	}
	for (i = n-1, j = 0; i >= 0, j < N; i--, j++) {			//inizializzazione valori di Y sulla netlist
